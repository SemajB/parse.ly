...
app.post('/playlists', (req, res) => {
  const params = Object.create(req.body);

  db.addPlaylist(params).then(() => {
    res.send('Song saved to playlist');
  });
});
...
app.get('/playlists', (req, res) => {
  ...
  db.getPlaylists()
    .then((playlists) => {
      ...
      // res.json(playlists);
      console.log(playlists);
      let resolvedPlaylists = [];
      playlists.forEach(({ name, songs }) => {
        const realSongs = [];
        songs.forEach((songPromise) => {
          songPromise.then((songArr) => {
            console.log(songArr[0]._doc);
            realSongs.push(songArr[0]._doc);
          });
          resolvedPlaylists.push({
            name,
            songs: realSongs,
          });
        });
